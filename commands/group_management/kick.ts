...
export async function kick(ctx) {
  ...
  if (!(groupName && username)) {
    ctx.telegram.sendMessage(ctx.message.chat.id, ARG_LEN_ERR_MESSAGE + "kick")
    return
  }
  
  const user = await userModel.findOne({uid: ctx.from.id, groupName: groupName, role: ["moderator", "admin"]})
  if (!user) {
  ...
  const kicked = await userModel.findOne({username: username, groupName: groupName, role: userPerms})
  if (!kicked) {
